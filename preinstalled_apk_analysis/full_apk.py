...
def odex_integration(odex_path, framework_path, apk_name, apk_path, output_path):
	...
	temp_folder = "temp"
	baksmali_command = "java -jar baksmali-2.4.0.jar deodex " + odex_path + " -d " + framework_path + " -o " + temp_folder
	os.system(baksmali_command)

	# convert smali to dex
	smali_command = "java -jar smali-2.4.0.jar assemble -o classes.dex " + temp_folder
	os.system(smali_command)
	shutil.rmtree(temp_folder)

	# use apktools to unzip apk
	apktools_disassemble = "java -jar apktool_2.4.1.jar d -s -r " + apk_path + " -o " + temp_folder
